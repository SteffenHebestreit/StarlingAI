...
| `DOC-504` | slice 1 landed (loop iteration 20) | Generated policy reference under `docs/reference/` — three files rendered from RUNTIME METADATA, never hand-edited: `tool-tiers.md` (every compile-time tier assignment incl. blocked tier-4, per-call approval + sandbox columns, from `listToolTierDefs()`), `deployment-modes.md` (the exact fail-closed dependency requirements and reasons `/readyz` reports, from `evaluateDeploymentReadiness`), `config-flags.md` (all 433 schema fields with declaration site + production read-site counts, from the feature-registry audit). `pnpm docs:reference` regenerates; renderers are deterministic (stable sorts, no timestamps) and CI fails on drift (`git diff --exit-code docs/reference/`) — enforcement can no longer change without the reference changing in the same PR. 4 tests. Also this iteration: full-suite regression over the day's five commits — **2,630 passed / 0 failed**. Remaining DOC-504 scope: tool effect-class column (needs SEC-106 effect contracts), metric definitions page, README/architecture cross-checks. |
| `SEC-106` | slice 1 landed (loop iteration 21) | Effect contracts per ADR-005, shadow-first. Tools may declare an `effect` block (domain, reversibility, dataClassification, resolved-target); behind new `guardrails.effectContracts` (off default / deployment **shadow** / enforce): every external-mutation call emits an `effect_receipt` audit (effectId, normalized target, request hash, approvalRequired, outcome **succeeded/failed/unknown**), and irreversible/compensatable external mutations demand approval in enforce (shadow records `effect_approval_would_block` on real traffic for calibration). `unknown` is STRUCTURAL — a `dispatchUncertain` flag on ToolResult set by the registry's own timeout race and by tools classifying their transport failures (abort mid-flight / connection lost after dispatch → uncertain; DNS/refused/auth → terminal failed) — never text matching. First wave: `webhook__*` (ALL webhooks irreversible unless the definition declares `readOnly: true` — the HTTP verb is not evidence of purity) and `send_email`. A **21-agent adversarial review of the slice confirmed 8 findings (3×P1)** before commit, all fixed: the original substring timeout heuristic made `unknown` unreachable for every production tool (and false-positived on error bodies mentioning timeouts); GET webhooks were wrongly auto-pure; and the email channel returned ok:true even when all SMTP attempts failed and the mail was dead-lettered, while blind 3× SMTP retries could double-send beneath one receipt — tool-path sends are now single-shot with the DeliveryResult surfaced honestly. 12 effect tests + 2 messaging tests. Remaining SEC-106 scope: standing grants (scoped/time-bounded/usage-limited), dry-run plumbing, compensation metadata + receipts into the mission store, browser submit/click + git-push/infra annotation waves, retry-guard consuming `unknown` receipts. |
| `MIS-202` | slice 1 landed (loop iteration 22) | Mission contracts with child narrowing (`swarm/mission-contract.ts`): a contract states what a scope MAY do — agent/tool grants, effect policy in SEC-106's vocabulary (max reversibility + allowed domains), deadline, budget envelope (0 = unlimited, BUD-203 semantics). Root contract per root session (idempotent, from the dispatch context: grants, timeout-derived deadline, configured budget ceilings); every delegation derives a NARROWED child — intersection on grants/domains, min on reversibility rank/deadline/budget dims, stop conditions are UNION (children add stops, never remove). **No-silent-widening invariant:** a request exceeding any parent bound is clamped to the parent AND reported (`contract_narrowing_clamped` audit, warn) — never granted, never dropped silently. Every attempt links to its effective contract (`delegation_contract_attached` audit + `contractId` in attempt metadata); gated on `mission.store !== "off"` (deployment shadow → active). 9 tests. Remaining MIS-202 scope: acceptance/stop propagation from the TurnPlan into the root contract, ENFORCEMENT of the contract at dispatch (grants are currently telemetry), durable contracts as mission events, workflow/federation propagation. |
| `EVD-303` | slice 1 landed (loop iteration 23) | Evidence migration parity. (1) Raw `FACT:` extractions now dual-write into the evidence ledger as **UNVERIFIED observed-tier claims** (agent assertion, no source provenance) — they can never outrank sourced evidence under EVD-302's authority ranking and surface in verification queues instead of masquerading as findings. (2) Parity sweep (`swarm/evidence-migration.ts`, fired once per successful non-eval turn from finalize): compares the legacy shared-facts store against the ledger, **backfills** legacy facts the dual-write missed as unverified migration claims (idempotent — converges instead of drifting), counts ledger-only rich claims separately, and emits `evidence_migration_parity` telemetry (warn when a backfill happened = the dual-write missed something). Reads stay legacy-authoritative; rollback = the ledger is still consumed by nothing answer-bearing. 4 tests. Remaining EVD-303 scope: value-level parity (same subject, diverging values), read-path cutover behind a flag once N clean sweeps observed, mission-scoped durable retention. |
| `SEC-105` | slice 2 landed (loop iteration 24) | Trust-before-load per ADR-007 (slice 1 = plugins default-off, shipped earlier). Plugin trust is recorded against the sha256 CONTENT-TREE digest (`plugin/trust.ts`: sorted relative paths + per-file hashes, path+content bound; >5000-file trees refuse fingerprinting = untrusted): with new `plugins.requireTrust` (default **ON** — fail closed) the loader fingerprints every plugin and refuses load BEFORE any import unless `plugins.trust` carries a matching `{name, digest}` receipt — module-level code in an untrusted or modified plugin never executes. Any byte change (source, vendored dep, rename) produces a new digest and silently revokes trust; the refusal log prints the computed digest so a reviewed change can be re-trusted deliberately. Audits: `plugin_trust_verified` / `plugin_load_refused_untrusted`. Full lifecycle test: refused-before-import (importer never invoked) → trusted digest loads → one-byte change re-refuses. Remaining SEC-105 scope (ADR-007): data-only manifest with declared capabilities, per-plugin worker process + capability-scoped RPC bridge, minimal-env isolation, crash/timeout/OOM containment, signature verification. |
| `SEC-106` follow-up | retry guard landed (loop iteration 25) | ADR-005 transition invariant wired: an IDENTICAL re-invocation (same session, tool, request hash) of a non-idempotent external mutation whose previous receipt recorded outcome **unknown** is refused in enforce mode (before approval/execution, with an explicit verify-first error) and recorded in shadow (`effect_retry_after_unknown` audit). A later TERMINAL receipt for the same request clears the marker; idempotent/pure effects are exempt by definition; the in-process marker ledger is TTL'd (10 min) and bounded (500). 3 lifecycle tests (refuse/record, terminal-clears, idempotent-exempt). Remaining: markers shared across processes (Redis) once enforce mode is deployment-real, operator resolution surface (UX-502). |
| `GRF-206` | slice 2 landed (loop iteration 26) | Restart scheduler, detection stage. Graph DEFINITIONS (shape: nodes, deps, tasks, objective; payload bounded) are now persisted at dispatch and deleted only on TERMINAL completion — a definition surviving a process death IS the crash evidence. Boot-time scanner (`swarm/graph-restart.ts`, behind new `mission.durableTaskGraph` off/shadow, deployment shadow) cross-references each surviving definition against the durable per-node ledger and audits a resume candidate per graph (`task_graph_interrupted_detected`: completed node ids to REUSE, pending node ids gated on operator review — a pending node's effect class is unknown until it runs, and blind re-dispatch is the double-mutation ADR-005 forbids). 4 tests (detect + classify, clean-completion deletes, multi-session, payload bounds). Remaining GRF-206 scope: the auto re-dispatch executor (headless graph resume honoring effect classes + unknown receipts — chaos-pack-gated per the plan), CAS status transitions, Merkle transitive node keys. |
| `SEC-105` | slice 3 landed (loop iteration 27) | ADR-007 manifest layer. Data-only `plugin.manifest.json` (identity, entry, declared tools, requested capabilities: network hosts + env vars) parsed and validated WITHOUT importing plugin code; optional for legacy plugins, ENFORCED when present — identity mismatch, missing/traversal entry, malformed JSON reject the plugin before the trust gate. Trust-time compatibility scanner (static heuristic over plugin source, bounded): flags gateway-state reliance that breaks under worker isolation — raw `process.env` (manifest-declared vars suppress their own findings), child processes, core-internal imports, `process.exit`, global mutation — as non-blocking `plugin_compat_risk` audits. 7 tests. Remaining SEC-105 scope: the worker-process host itself (minimal-env child + capability-scoped RPC + crash/timeout containment), signature verification. |
| `SEC-105` | slice 4 landed (loop iteration 28) | ADR-007 worker isolation. With `plugins.isolation: "worker"` (default `in_process`), each trusted plugin imports inside its OWN child process with a MINIMAL environment — baseline OS vars plus manifest-declared env names only; gateway secrets (JWT/master/provider keys) and backend URLs are structurally invisible. Newline-JSON stdio protocol (`worker-main.ts`, dependency-free entry); the host (`worker-host.ts`) provides init-handshake + per-invoke timeouts and crash containment: worker death → all pending calls rejected, plugin DEGRADED (fast-fail), `plugin_worker_crashed` audit — the gateway never sees the crash as an exception. Tools RPC in through an importer with the in-process importer's shape, so the loader's validation path is reused unchanged. Proven by 5 REAL child-process tests: secrets invisible in-worker (jwt/redis null, declared var visible), hung call times out while the worker keeps serving, crash degrades without touching the test process, import-time explosion rejected at handshake. **13-agent adversarial review: 8 confirmed findings (2 P1), all fixed pre-commit** — hot-reload resync respawned workers for already-loaded plugins and bricked their registered tools (already-loaded skip now precedes any import), unhandled stdin EPIPE could crash the GATEWAY via uncaughtException (stream error listeners + guarded writes → degrade), exit-before-ready stalled the full 15s init timeout (degrade now rejects pending init), unloaded/rejected plugins leaked live workers (per-id disposal on unload + rejected loads + gateway shutdown), intentional disposal audited as a crash (disposed flag). Remaining SEC-105 scope: capability-scoped RPC bridging (fetch/fs proxies to declared targets), memory caps, signature verification, `trustedLegacyInProcess` sunset tooling. |
| `EVL-402` | slice 2 landed (loop iteration 29) | Chaos-crash pack. A REAL child process runs a task graph against real Redis, completes one node durably, then hard-exits 137 with no cleanup (what kill -9/OOM looks like); a DIFFERENT process — playing next boot — must detect the wreck via `scanForInterruptedTaskGraphs` and classify it exactly (completed `research` reused, `build`/`verify` pending). Companion scenario proves clean completion leaves nothing. Registered as the `chaos-crash` deterministic pack (publishes the unified envelope; self-skips without REDIS_URL — CI's packs step provides the service container); validated locally 2/2 against a throwaway Redis. The weekly load suite (saturation/exhaustion under load) remains the `chaos-load` placeholder. |
| `GRF-206` | slice 3 landed (loop iteration 30) | Safe resume planner. Durable per-node START markers (`writeTaskGraphNodeStarted`, written the instant a node begins dispatch, before it runs; cleared with the definition on clean completion) let the boot scanner distinguish, among not-completed nodes, those that were IN-FLIGHT at the crash (marker, no completion → effect UNKNOWN, may have fired an irreversible external effect) from those that NEVER STARTED (no marker → provably fired no effect). `planGraphResume` classifies each node and computes the safe re-dispatch frontier: never-started nodes whose deps are all completed. In-flight nodes are NEVER on the frontier and the audit's `resumeAction` becomes `operator_review_required` when any exists — exactly ADR-005's unknown-outcome rule (blind replay forbidden). The `chaos-crash` pack now marks a node in-flight before the kill and asserts the cross-process classification (build in-flight, verify never-started, nothing resumable). 6 unit tests. Remaining GRF-206 scope: the HEADLESS resume executor that actually re-dispatches the safe frontier at boot (needs a synthetic session/tool context; deliberately deferred as the boot-behavior-changing step — the safety classification it depends on is now in place). |
| `SEC-105` | slice 5 landed (loop iteration 31) | ADR-007 network capability guard. Inside the isolated worker, before the plugin's module code imports, `installNetworkGuard` (`plugin/capability-guard.ts`) wraps global `fetch` to a **deny-by-default** exact-host allowlist drawn from the manifest's `capabilities.network`: a plugin that declared no network capability cannot fetch at all; one that declared hosts reaches only those exact hosts (no suffix trickery — `api.example.com.evil.com` is blocked). Unparseable URLs fail closed. The declared hosts thread manifest → loader → `spawnPluginWorker` → worker init. 7 unit + 2 real-worker end-to-end tests (no-capability worker can't fetch; declared-host worker blocked for other hosts). **Honestly scoped as defense-in-depth, NOT a hard boundary** — it gates the common fetch egress path but a plugin using `node:http` directly bypasses it; true network isolation is the container tier for `untrusted_multi_tenant` (the digest-pinned trust gate already ensures only operator-approved code runs here). Remaining SEC-105 scope: fs subtree sandbox (Node permission model at spawn — conflicts with tsx dev mode, deferred), signature verification, `trustedLegacyInProcess` sunset tooling. |
