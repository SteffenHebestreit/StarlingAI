...
const ROUTING_FIXTURE = {
  ...
  subAgents: {
    ...
    code_analyst: {
      description: "Reviews, explains, and retrieves code. Answers questions about code structure and logic; locates files, symbols, references, and exact values via workspace search.",
      capabilities: ["code review", "code analysis", "code explanation", "architecture review", "workspace search", "code retrieval", "symbol lookup", "reference tracing"],
      tags: ["code", "analysis", "retrieval", "workspace", "search", "references"],
    ...
    ops_triage: {
      description: "Operational triage and runtime-incident agent. Diagnoses service health, deployment failures, container/runtime issues, provider connectivity, MCP server failures, channel errors, gateway health, and configuration drift. Reads logs, checks endpoints, and produces a structured incident report with recommended remediation.",
      capabilities: ["service triage", "runtime incident triage", "incident triage", "health diagnosis", "provider troubleshooting", "gateway diagnosis", "log analysis", "deployment diagnosis"],
      tags: ["ops", "monitoring", "incident", "reliability", "triage", "provider", "gateway"],
...
const ROUTING_CASES: Array<{ query: string; expected: string; description: string }> = [
  ...
  { query: "search the workspace for all files that reference the AuthService class", expected: "code_analyst", description: "workspace code search" },
  { query: "find every mention of the database connection string in the project", expected: "code_analyst", description: "workspace retrieval" },
  { query: "the LM Studio provider keeps returning 502 errors, diagnose the issue", expected: "ops_triage", description: "provider incident triage" },
  { query: "gateway health check is failing and agents are timing out", expected: "ops_triage", description: "runtime health diagnosis" },
