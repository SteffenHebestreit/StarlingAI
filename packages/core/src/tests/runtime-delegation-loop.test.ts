...
describe("runtime delegated-loop regressions", () => {
  ...
  it("returns a cached result instead of re-executing an identical tool call across iterations", async () => {
    ...
    streamMock.mockImplementation(() => {
      llmCallCount += 1;
      if (llmCallCount === 1) {
        return createToolCallStream("read_1", "read_file", { path: "workspace/README.md" });
      }
      if (llmCallCount === 2) {
        return createToolCallStream("read_2", "read_file", { path: "workspace/README.md" });
      }
    ...
    const readFileMock = vi.fn(async () => ({
      success: true,
      output: "workspace readme content",
    ...
    const result = await freshRuntime.runTurn({
      session,
      // Neutral scenario: this test exercises generic identical-tool-call caching
      // (mock-driven, message-independent). The prior "pentest workflow file" wording
      // is avoided so the caching assertion never couples to routing on those words.
      userMessage: "Review the workspace README file.",
      onToolResult: (_toolCallId, _name, toolResult) => observedToolResults.push(toolResult),
    });
