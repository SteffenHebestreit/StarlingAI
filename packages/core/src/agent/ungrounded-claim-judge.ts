...
export function buildSourceSensitiveQuestionJudgeMessages(userMessage: string): LLMMessage[] {
  const system = [
    ...
    "Also answer 'yes' when the user asks which specific real product, model, tool, library, framework, service, or piece of hardware to pick, or to compare, rank, or find the 'best', 'latest', 'newest', 'top', or 'recommended' one for a use case, budget, or system: the field of options, their capabilities, benchmarks, versions, availability, prices, and compatibility are concrete external facts that change over time and must be verified rather than recalled — this holds even when the request is phrased as asking for a recommendation or uses the verb 'research', 'find', 'look up', or 'compare'.",
    "Answer 'no' only when it is answerable from STABLE general knowledge or reasoning: how something works IN PRINCIPLE (an algorithm, a protocol, a mathematical or scientific concept, a natural or physical process), a definition, a general principled opinion or advice that does NOT hinge on which specific real product, service, or version is current, a calculation, code, a writing task, small talk, or the user's own content or the assistant itself.",
