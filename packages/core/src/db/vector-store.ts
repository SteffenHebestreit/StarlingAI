...
function getPool(): pg.Pool | null {
  ...
  if (!url) return null;
  // This pool is shared by skills + memory + RAG + ingest, all hit concurrently
  // under parallel sub-agent retrieval — the old hardcoded max:3 was a hidden
  // concurrency ceiling (queries queued behind 3 connections). Default 10, tunable
  // via SAI_PGVECTOR_POOL_MAX (reproducible through the .env shard).
  const max = Math.max(1, Number.parseInt(process.env["SAI_PGVECTOR_POOL_MAX"] ?? "10", 10) || 10);
  _pool = new Pool({ connectionString: url, max });
  return _pool;
}
...
/**
 * A vector is unusable for cosine similarity when it is all-zero (undefined
 * direction → NaN distance in pgvector) or carries any non-finite component
 * (a truncated / failed embed response). Storing such a vector silently makes
 * the chunk permanently unretrievable, so we reject it at the store boundary
 * instead. Exported for testing.
 */
export function isDegenerateVector(v: Float32Array | number[]): boolean {
  let allZero = true;
...
export async function vectorUpsert(entry: VectorUpsert): Promise<boolean> {
  ...
  if (!vec || vec.length !== _dim || isDegenerateVector(vec)) return false;
...
/**
 * Batch upsert. Returns the number of rows written. Builds ONE multi-row INSERT
 * via unnest() instead of N serial round-trips (RAG ingest writes one chunk per
 * call against a small pool). De-dups (collection,id) last-wins so ON CONFLICT
 * doesn't self-collide within the batch; falls back to the per-row loop on error.
 */
export async function vectorUpsertMany(entries: VectorUpsert[]): Promise<number> {
  if (entries.length === 0) return 0;
  if (!_ready && !(await initVectorStore())) return 0;
  const pool = getPool();
  if (!pool) return 0;

  const prepared = new Map<string, { collection: string; id: string; content: string; metadata: string; vec: string }>();
  ...
  try {
    // Pass every column as text[] and cast per-row (jsonb/vector) — node-pg encodes
    // string[] as a safely-escaped Postgres array literal, mirroring the single-row
    // `$4::jsonb` / `$5::vector` casts. unnest also dodges the 65535-bind ceiling.
    await pool.query(
      `INSERT INTO ${TABLE} (collection, id, content, metadata, embedding)
       SELECT collection, id, content, metadata::jsonb, embedding::vector
       FROM unnest($1::text[], $2::text[], $3::text[], $4::text[], $5::text[])
         AS t(collection, id, content, metadata, embedding)
       ON CONFLICT (collection, id)
       DO UPDATE SET content = EXCLUDED.content, metadata = EXCLUDED.metadata, embedding = EXCLUDED.embedding, created_at = NOW()`,
      [
    ...
    log.warn({ err, count: rows.length }, "batch vector upsert failed — falling back to per-row");
    let written = 0;
    for (const entry of entries) {
