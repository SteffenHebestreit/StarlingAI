...
const props = withDefaults(defineProps(), { aiState: "default" });
...
function resolveVisuals(stateName) {
...
let orbGroup; // scaled root for the whole orb
let torusPoints;
let sphereGroup;
let torusUniforms;
let sphereUniforms;
...
function buildTorusGeometry() {
    ...
    while (indexSet.size < NUM_NODES)
        indexSet.add(Math.floor(Math.random() * PARTICLE_COUNT));
...
function animate() {
    ...
    if (transitionProgress < 1) {
        ...
        if (t > 0.5) {
            cv.pulsesFromCenter = dst.pulsesFromCenter;
            cv.frozenAnimation = dst.frozenAnimation;
        }
        ...
        sphereUniforms["uNeuralSphereColor"].value.copy(cv.neuralSphereColor);
    ...
    if (time - lastPulseTime > nextPulseInterval / cv.pulseRateFactor && activePulses.length < MAX_ACTIVE_PULSES) {
        ...
        if (cv.pulsesFromCenter) {
            ...
            const bp = geo.attributes["particlePosition"].array;
    ...
    const pulseOrigins = torusUniforms["uPulseOrigins"].value;
    const pulseRadii = torusUniforms["uPulseTravelRadii"].value;
    ...
    // ── Neural sphere ─────────────────────────────────────────────────────────────
    if (!cv.frozenAnimation)
        neuralSphereRotation = time * NS_ROTATION_SPEED;
...
onMounted(() => {
    ...
    torusUniforms = {
        ...
        uPulseTravelRadii: { value: new Array(MAX_ACTIVE_PULSES).fill(0) },
...
onUnmounted(() => {
    ...
    torusPoints?.material?.dispose();
    const spherePoints = sphereGroup?.children[0];
...
const __VLS_withDefaultsArg = (function (t) { return t; })({ aiState: "default" });
const __VLS_ctx = {};
