...
```
If your model server runs on a DIFFERENT host than the eval (e.g. the model lives on a homelab box
and you run the CLI from a dev machine), also set `SAI_PRIMARY_MODEL_URL=http://<host>:1234/v1`
(the provider-neutral primary-model var; `SAI_LMSTUDIO_URL` is the back-compat alias). The baked
config default is `host.docker.internal:1234`, which from the CLI means *this* machine, so without
the override the run dials the wrong host and every case crashes with `Sub-agent error:` (the
pre-flight check now catches this and aborts with a clear message instead).
